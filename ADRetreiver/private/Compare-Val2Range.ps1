...
function Compare-Val2Range ([int] $value, [string] $range) {
  ...
  if ($range -notmatch $inteReg) {
    throw "Error: $range is not a valid range value ! Please follow the exemples: [30;90[, [30;90] or ]30;90["
  }
  ...
  $firstCondition = if ($range[0] -eq '[') { $value -ge $firstValue } else { $value -gt $firstValue }
  $secondCondition = if ($range[-1] -eq ']') { $value -le $secondValue } else { $value -lt $secondValue }
